...
No result from the physical display sequence has been recorded yet. CI runs an Ubuntu 24.04 systemd transaction with a stub wall process, including a named instance upgrade, rollback, and injected health failure. A second CI trial packages the real amd64 binary, installs it on the runner, probes a local H.264 RTSP pattern, validates generated and hand-written YAML, applies the hand-written file to an Xvfb window sink, checks output and decoded-frame counters, then applies a deliberately missing RTSP path and checks that rollback restores the prior YAML and live frames. It uses a SHA-256-pinned MediaMTX v1.21.1 test server and an eight-minute limit. This trial [passed on the Ubuntu 24.04 runner on 2026-09-25](https://github.com/sapireli/eufy-p2p-rtsp-bridge/actions/runs/36140418876/job/108088589200). Xvfb progress does not establish visible HDMI pixels, DRM connector and plane behavior, Pi decoder capacity, camera wake behavior, or recovery on physical hardware; record those separately in the render and recovery trial below.
...
gst-inspect-1.0 --exists watchdog
gst-inspect-1.0 --exists videoconvert
gst-inspect-1.0 --exists videoscale
gst-inspect-1.0 --exists videorate
...
If the screen is black, inspect `eufy-wall doctor`, `journalctl`, output ownership (`video` and `render` groups), and whether another compositor holds DRM. On Debian, `appsink`, `appsrc`, `compositor`, `videoconvert`, `videoscale`, and `videorate` come from `gstreamer1.0-plugins-base`; `watchdog` comes from `gstreamer1.0-plugins-bad`. The native app relay also needs `libgstapp-1.0.so.0` from `libgstreamer-plugins-base1.0-0`. A camera that stops delivering frames should be detected by the runtime progress watchdog; confirm the recovery in the journal. If an upgrade fails, use the installer `--rollback` and inspect the current binary symlink with `readlink /opt/eufy-wall/current`.
