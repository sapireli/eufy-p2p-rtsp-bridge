...
If an imported file is unversioned, the editor shows a migration summary with the derived `bridge_url`, a custom canvas matching the old grid, tile IDs, and explicit rectangles. Type `MIGRATE` to accept an in-memory v2 draft, then `save` or `apply`; any other answer leaves the original file untouched. `eufy-wall config migrate legacy.yaml --output candidate.yaml` offers the same conversion for scripts and SSH sessions, with no active config change. Migration rejects unknown fields that it cannot preserve. Verify the derived bridge URL before applying. A v2 preset such as `layout: 2x2` is valid to run, but the editor requires `layout: custom` so every edit has an explicit rectangle.
