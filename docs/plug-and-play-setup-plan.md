...
- For planes, every active tile needs a usable plane that can reach the selected CRTC. The CLI must discover and probe those plane/connector combinations; it cannot merely count IDs. Current validation confirms a named connector and decoded source frames, but plane-to-CRTC reachability and visible multi-tile output still need physical render qualification. A compositor profile has its own decode and memory budget; its current native renderer switches sources per tile, pending hardware measurements.
