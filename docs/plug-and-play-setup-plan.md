...
Status: implementation in progress on `plan/plug-and-play-setup`, 2026-09-25. The acceptance criteria below remain the release gates. Node and Go CI require more than 80% overall line or statement coverage with adversarial and unit tests. The integrated setup commands, config transactions, inventory contract, full-screen and line-mode layout editors, verified Linux installers, native per-tile compositor recovery, and macOS window/launchd test artifacts are implemented on this branch. An Intel macOS window trial kept output advancing through two 45-second RTSP publisher outages and recovered decoded frames twice; a separate live motion tile trial switched H.264→H.265→H.264 while a fixed peer kept decoding. A 30-minute soak on the final-source Intel binary kept sampled output and decode at or above 14.9 fps without a harness failure. Ubuntu CI now exercises packaged server and client installs, manual YAML, client live RTSP frames, and failed-apply rollback with synthetic dependencies. Full Pi/Debian/macOS hardware qualification, clean-host usability and installer evidence, real-camera soak evidence, and Apple signing/notarization remain open; the runbooks mark unmeasured profiles unverified.

Local verification on 2026-09-25: the Node bridge passed 190 tests with three platform skips and 92.37% first-party line coverage; the Go client passed 290 tests under the race detector with 80.8% aggregate statement coverage, `go vet`, `gofmt`, and all Linux/macOS cross-builds. Linux and macOS installer suites passed, including an install/upgrade/rollback trial of the packaged Intel Mac binary with launchd stubbed. The [final-source Intel Mac soak evidence](evidence/macos-intel-2026-09-25-final-source-soak.json) records the binary digest and measured limits. These checks establish software behavior on the tested hosts, not physical target qualification.

The [six-job PR CI run at `3526d18`](https://github.com/sapireli/eufy-p2p-rtsp-bridge/actions/runs/36140418876) passed on 2026-09-25: 196 Node tests with 92.55% first-party line coverage, Linux Go race tests with 80.4% aggregate statement coverage, Linux client and server package trials, and Intel/Apple Silicon macOS jobs. The [Intel Mac codec and compositor fault trial](evidence/macos-intel-2026-09-25-codec-compositor-fault.json) records the measured switch and recovery limits. CI runs and synthetic media tests do not establish physical display or real-camera support.

Open investigation: a headless macOS native-test startup decoded source frames without producing compositor output for more than six seconds. The renderer now checks initial compositor output and retries a stalled pipeline; 30 subsequent headless startups passed. A separate cold-start hang traced to AppKit calls on a non-main OS thread; locking the Darwin startup thread resolved it in 40 fresh-home starts. The underlying headless GStreamer stall, physical window continuity, and clean-host service restart under that condition still need qualification; see the macOS runbook.
...
The default `decoder: auto` makes a choice for each source codec on the display host. It prefers hardware VideoToolbox on macOS, V4L2 on Raspberry Pi, and VA on a Debian host with a usable render device; it uses software when that codec has no available hardware decoder. Pi 5 therefore decodes HEVC in hardware and H.264 in software. Doctor and startup output name the selected elements. For the native compositor/window, recent compressed input followed by a decoder error or sustained decoded-frame stall switches only the affected tile to software, and runtime status records the actual element. Physical rejection tests, pre-buffer negotiation failures, a bounded return to hardware, and the separate planes-runtime fallback remain open.
...
The editor does not try to draw all 32×32 cells as text on a small terminal. It scales rectangles into a viewport and always shows exact grid coordinates and pixel dimensions in the property panel. It supports terminals at least 80×24, has a linear menu mode for smaller or screen-reader sessions, and never requires mouse input. `eufy-wall layout preview <file> --display` renders numbered colored rectangles and letterboxing on the selected Linux display or a macOS desktop window without opening camera streams. `--png <path>` creates the same preview for SSH sessions. A short optional live probe can be run after validation to confirm decoder and RTSP behavior.
...
- For planes, every active tile needs a usable plane that can reach the selected CRTC. The CLI checks DRM connector/encoder/CRTC/plane routing with `modetest` and decodes fixed live sources, but format/scaling compatibility and visible multi-tile output still need physical render qualification. A compositor profile has its own decode and memory budget; its current native renderer switches sources per tile, pending hardware measurements.
...
5. Replace `gst-launch` as the live planes supervisor with a native GStreamer runner that owns one pipeline per plane. Probe compressed input at the decoder and decoded output, classify decoder bus errors, and switch only the affected `decoder: auto` tile to an available software decoder when active input proves a hardware failure. Keep explicit decoder choices fixed and preserve peer tiles during recovery. Test decoder rejection, silent stalls, and publisher outages in headless pipelines before qualifying real DRM planes; command-line output from `gst-launch` is not a reliable failure classifier.
