...
1. Snapshot a clean VM. Install A with the verified command above. Confirm `/opt/eufy-wall-bridge/current/VERSION` is A, `/etc/eufy-wall-bridge.yaml` and `.env` exist, the env mode is `0600`, and `systemctl is-active --quiet eufy-wall-bridge` and `systemctl is-enabled --quiet eufy-wall-bridge` both return nonzero. Repeat the same install and confirm the unit hash and service state do not change.
