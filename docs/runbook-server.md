...
A partial clean-runner trial [passed on Ubuntu 24.04 amd64 on 2026-09-25](https://github.com/sapireli/eufy-p2p-rtsp-bridge/actions/runs/36140418876/job/108088589084). It installed the real packaged Node 24.5.0, go2rtc, SDK, CLI, and systemd unit from a verified local archive; confirmed the root-only `0600` environment file and stopped service; printed the example as an ordinary user; validated manual YAML from a file and stdin as root; and confirmed that a failed apply restored the previous YAML and reported the rollback. The apply trial used a synthetic local `/healthz` response and a restart-only `systemctl` stub. It did not start the bridge, authenticate a Eufy account, test a camera, or exercise the full A/B upgrade and offline sequence above. A complete Debian clean-host result remains open; keep it separate from camera soak and display qualification.
