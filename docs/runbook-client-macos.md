...
Install [Homebrew GStreamer](https://formulae.brew.sh/formula/gstreamer) with `brew install gstreamer`. The current formula includes the GStreamer plugins. Confirm `gst-launch-1.0`, `gst-inspect-1.0`, `compositor`, `autovideosink`, `watchdog`, `appsink`, `appsrc`, `videoconvert`, `videoscale`, and `videorate` are available. The app elements relay each tile's frames to a persistent compositor input; the video elements convert, scale, and pace frames. The installer also checks `libgstapp-1.0.dylib` for the native relay. Install a recent [GitHub CLI](https://cli.github.com/) and authenticate it as your login user (`gh auth login` or `GH_TOKEN`) for online attestation lookup. Run the commands below in Terminal in that user's desktop session, with no `sudo`.
