...
Install [Homebrew GStreamer](https://formulae.brew.sh/formula/gstreamer) with `brew install gstreamer`. The current formula includes the GStreamer plugins. Confirm `gst-launch-1.0`, `gst-inspect-1.0`, `compositor`, `autovideosink`, `watchdog`, `intervideosrc`, `intervideosink`, `videoconvert`, `videoscale`, and `videorate` are available. The intervideo elements connect each tile source to the compositor; the video elements convert, scale, and pace frames. Install a recent [GitHub CLI](https://cli.github.com/) and authenticate it as your login user (`gh auth login` or `GH_TOKEN`) for online attestation lookup. Run the commands below in Terminal in that user's desktop session, with no `sudo`.
