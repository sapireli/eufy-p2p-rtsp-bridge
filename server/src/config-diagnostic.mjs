...
export function configDiagnostic(error) {
  ...
  const secrets = error?.code === "CONFIG_SECRETS_UNREADABLE";
  ...
  const code = usage ? "CLI_USAGE" : yaml ? "CONFIG_YAML_INVALID" : unsupported ? "CONFIG_UNSUPPORTED_KEY" :
    ...
    apply ? "CONFIG_APPLY_ROLLED_BACK" : secrets ? "CONFIG_SECRETS_UNREADABLE" : file ? "CONFIG_FILE_UNREADABLE" : "CONFIG_INVALID";
  const remedy = usage ? "Follow the shown command syntax and rerun it." :
    ...
    secrets ? "Run this command with sudo to read the installed root-owned credentials file, or set BRIDGE_ENV to a readable file." :
